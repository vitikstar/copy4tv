...
class NovaPoshtaApi2
{
    ...
    public function getWarehouseName($city="",$site_key=0){
        $city_data = $this->getCity($city);
        ...
        $result = $this->getWarehouses($Ref);
        if($result){
            foreach ($result['data'] as $k=>$item) if($site_key == $item['SiteKey']) return $item['Description'];
        }else{
            return $site_key;
        }
    }
