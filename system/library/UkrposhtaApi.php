...
class UkrposhtaApi
{
    ...
    {
        /* Get required URL*/
        $url = $this->url . 'ecom' . $this->apiVersion . $model . $add . '&token=' . $this->token . "&get_postoffices_by_postindex&pi=23610";
//echo $url;
//exit;
        /* Convert data to neccessary format*/
        $post = json_encode($params);
    ...
    public function parcelList($model, $params = NULL, $add = '')
    {
        /* Get required URL*/
        $url = $this->url . 'ecom' . $this->apiVersion . $model . $add . '?token=' . $this->token;
     ...
     * @return array|mixed
     */
    function modelSmartBoxPut($smartboxshipmentuuid)
    {
        return $this->requestTokenPut('shipments', null, '?uuid=' . $smartboxshipmentuuid);
    }
