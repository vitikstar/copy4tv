...
class Debug
{
    ...
    public static function log($str, array $params = array())
    {
        ...
        self::$_log[] = htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
    }
    ...
    public static function show($sysstart,$db,$request, $session)
    {
      ...
      //  var_dump($db);
        ...
        $p ='';
        if (isset($profs)) {
            foreach ($profs->rows as $prof) {
                $style = ($prof['Duration'] > 0.009) ? 'style="background:#9B0404; color: #fff"' : '';
                $p .= '<tr valign="top"><td '.$style.'>' . $prof['Query_ID'] . ' </td><td '.$style.'> ' . $prof['Query'] . ' </td><td class="tRight" '.$style.'> ' . $prof['Duration'] . ' s</td></tr>';
        ...
        <div id="ptb_data_cont_sql" class="ptb_data_cont">
            <ul class="ptb_tabs">
              <li id="ptb_tab_sqldefault">profiles sql <span>(' . $prof['Query_ID'] . ')</span></li>
            </ul>
        ...
        $f .= '<tr class="total">
                    <td></td>
                    <td>total ' . $prof['Query_ID'] . ' queries</td>
        ...
        <div id="ptb">
            <ul id="ptb_toolbar" class="ptb_bg">
                ...
                <li class="sql"><span class="icon"></span> sql <span class="total">(' . $prof['Query_ID'] . ')</span></li>
