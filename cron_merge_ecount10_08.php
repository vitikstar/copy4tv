        ...
        foreach ($result->rows as $customer_info) {
            ...
            if (!$query->rows and !$query2->rows) {
                ...
                if ($customer_id != $customer_info['customer_id']) {
                    ...
                    $result = $db->query("SELECT * FROM oc_customer WHERE relation_google!='0' AND customer_id='" . $customer_info['customer_id'] . "'");

                    if ($result->rows) {
                        $sql = "UPDATE oc_customer SET relation_google='0' WHERE customer_id='" . $customer_info['customer_id'] . "'";
                        $db->query($sql);

                        $result2 = $db->query("SELECT * FROM oc_customer WHERE relation_google='0' AND customer_id='" . $customer_id . "'");
                        if ($result2->rows) {
                            $sql = "UPDATE oc_customer SET relation_google='" . $result->row['relation_google'] . "' WHERE customer_id='" . $customer_id . "'";
                            $db->query($sql);
                        }
                        $sql = "UPDATE oc_customer SET relation_google='1' WHERE customer_id='" . $customer_id . "'";
                        $db->query($sql);
                    }
