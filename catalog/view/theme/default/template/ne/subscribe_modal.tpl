<div class="ne-bootstrap ne-modal-wrapper <?php echo $class; ?> ne-modal<?php echo $module; ?><?php if ($popup_delay) { ?> ne-popup-inactive<?php } ?>" tabindex="-1">
...
jQuery(function ($) {
    ...
    if (!$.cookie('ne_subscribed') && !$.cookie('ne_days')) {
        var ts = (new Date()).getTime() + <?php echo $popup_timeout; ?>*1000,
            timer = $('#ne-timer<?php echo $module; ?>');
        ...
        <?php if ($popup_timeout) { ?>
            $('#ne-modal<?php echo $module; ?>').on('shown.bs.modal', function () {
                $('#ne-timer<?php echo $module; ?>').countdown({
                    timestamp: ts,
                    callback: function(days, hours, minutes, seconds) {
                        timer.text(days * 86400 + hours * 3600 + minutes * 60 + seconds * 1);
                        if (!minutes && !seconds && !days && !hours) {
                            $('#ne-timer<?php echo $module; ?>').replaceWith('<a href="#" class="ne-modal-close" data-dismiss="modal"><span>&times;<span></a>');
                            return true;
                        }
                    }
                });
