	...
	class ControllerCommonSeoBlog extends Controller {
		...
		public function __construct($registry) {
			...
			if ($this->cache_rewrite_flag) {
				$this->cache_rewrite_file = 'blog.seoblog_rewrite.' . $this->config_scheme . '.' . (int) $this->config->get('config_store_id') . '.' . (int) $this->config->get('config_language_id');
		...
		public function index() {
			...
			if (isset($this->request->get['_route_'])) {
				...
				if (isset($this->request->get['record_id']) && $this->request->get['record_id'] != '') {
					array_push($parts, 'record_id=' . (int)$this->request->get['record_id']);
				}
				...
				if ($flg) {
					$layout_id = $this->seocmslib->getLayoutIdSeocms($this->request->get['route']);

                    $this->registry->set('sc_layout_id', $layout_id);
				...
				*/
				...
				if ($url) {
					...
					if ($data) {
						...
						$query =  $this->getQueryString($data, array('page'));
		...
		private function validate() {
			...
			$seo = urldecode(str_replace('&amp;', '&', str_replace($this->config_url, '', $this->url->link($this->request->get['route'], $this->getQueryString($this->request->get, array(
				'route',
				'_route_',
				'site_language'
			)), $this->url_link_ssl))));
			...
			if (rawurldecode($url) != rawurldecode($seo)) {
				if (!$this->data['settings_general']['comp_url']) {
					if (SC_VERSION < 20) {
						header($this->request->server['SERVER_PROTOCOL'] . ' 301 Moved Permanently');
						$this->response->redirect($this->config_url . $seo);
					} else {
						$this->response->redirect($this->config_url . $seo, 301);
					}
				}
			}
		}
