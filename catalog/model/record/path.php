...
class ModelRecordPath extends Model
{
	public function pathbyrecord($record_id) {
		$cache_file = 'blog.path.record.' . (int) $this->config->get('config_store_id') . '.' . (int) $this->config->get('config_language_id');
	...
	public function pathbyblog($blog_id) {
		$cache_file = 'blog.path.blog.' . (int) $this->config->get('config_store_id') . '.' . (int) $this->config->get('config_language_id');
	...
	public function pathbycategory($category_id) {
		$cache_file  = 'blog.path.category.' . (int) $this->config->get('config_store_id') . '.' . (int) $this->config->get('config_language_id');
	...
	public function pathbyproduct($mark_id = 0) {
		...
		static $path = null;
		if (!is_array($path)) {
			$path = $this->cache->get('blog.path.product');
			if (!is_array($path))
				$path = array();
		}
		if (!isset($path[$mark_id])) {
			...
			$this->cache->set('blog.path.product', $path);
		}
		return $path[$mark_id];
	}
}
